    ...
    try:
        ...
        if int(n_records) > 0:
            ...
            if int(H_type_claims) > int(M_type_claims):
                ...
                resp="It was both non-par and par claim"
...
def get_proc_time(value1):
    ...
    try:        
        ...
        print(n_records)
        if int(n_records) > 0:
            pipe = [{'$match':{"root_claim_num":str(claim_num_rec)}},{'$group': {'_id':'$root_claim_num', 'total': {'$max': {'$convert':{'input':'$fromdatetoreceiveddate','to':'int'}}}}}] ## sum after type conversion of field
            mydoc=(mycol.aggregate(pipeline=pipe)) 
            fr="0"           
            for x in mydoc:
                fr=x['total']
            ...
            resp="<ul><li>The provider took "+fr+" days to submit the claim.</li>"+"<li>The UHC took "+rp+" days to pay the claim.</li>"+"<li>It took "+fp+" days for the claim to get paid.</li></ul>"
        else:
            resp="Could not find claim with Claim Number "+claim_num_rec+", Please make sure you entered correct details "
        return(resp)
    except Exception as e:
...
def get_received_claims(dt,provider_lst):
    ...
    try:
        ...
        if len(dt)==2:
            s1=str(dt[0])
            start = datetime.datetime(int(s1[0:4]),int(s1[5:7]),int(s1[8:10]),18,30)
            s1=str(dt[1].split('}')[0])
            end = datetime.datetime(int(s1[0:4]),int(s1[5:7]),int(s1[8:10]),18,30)
            print(start)
            print(end)
...
def get_paid_cnt_claims_hgh_dlr(dt,provider_lst):
    ...
    try:
        ...
        recvd_num=str(billed_amt)+" claims got paid out of "+ get_hgh_dlr_claims_cnt(dt).split('}')[0].strip(' ') +dy_op+mon_op+yr_op+q_op
        return recvd_num
