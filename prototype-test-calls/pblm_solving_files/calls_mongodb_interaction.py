...
def get_call_type(call_id):
    ...
    try:
        ...
        mydoc = mycol.find({"call_id": str(call_id)})
        for x in mydoc:
            return "The call is related to "+str(x['category'])+" category"
        return "Sorry, Could not fetch you results at this time"
...
def get_call_lob(call_id):
    ...
    try:
        ...
        mydoc = mycol.find({"call_id": str(call_id)})
        for x in mydoc:
            return "The call pertains to "+str(x['lob_id'])+" line of business"
        return "Sorry, Could not fetch you results at this time"
...
def get_call_product_type(call_id):
    ...
    try:
        ...
        for x in mydoc:
            try:
                return "The Product "+str(x['producttypeverbose']).split('-')[1].strip(' ')+" was enquired in call"
            except:
                return "The Product "+str(x['producttypeverbose']).strip(' ')+" was enquired in call"
        return "Sorry, Could not fetch you results at this time"
    ...
    try:
        ...
        for x in mydoc:
            if "no" in str(x['transfer']).lower() and "transfer" in str(x['transfer']).lower():
                return "No, the call did not get transfer"                
            else:
...
def get_call_BU(call_id):    
    ...
    try:
        ...
        mydoc = mycol.find({"call_id": str(call_id)})
        for x in mydoc:
            return "The call was related to "+str(x['bu'])+" business unit"
        return "Sorry, Could not fetch you results at this time"
...
def get_top_calls_talk_time_int(dur_lst,n):
...
def get_top_calls_ring_time_int(dur_lst,n):
...
def get_top_calls_hold_time_int(dur_lst,n):
...
def get_top_calls_total_time_int(dur_lst,n):
