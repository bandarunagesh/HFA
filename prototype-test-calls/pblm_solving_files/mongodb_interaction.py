...
def get_status(value1):
    ...
    try:
        ...
        if int(n_records)==int(n_denial_recds) and int(n_records)!=0:
            ...
            # print(denial_amt)
            pipe = [{'$match':{"root_claim_num":str(claim_num_rec)}},{'$group': {'_id':'$root_claim_num', 'total': {'$max': {'$convert':{'input':'$received_date','to':'date'}}}}}] ## sum after type conversion of field
            mydoc=(mycol.aggregate(pipeline=pipe))            
            t="--"
            for x in mydoc:
            ...
            # print(denial_amt)
            pipe = [{'$match':{"root_claim_num":str(claim_num_rec)}},{'$group': {'_id':'$root_claim_num', 'total': {'$max': {'$convert':{'input':'$paid_date','to':'date'}}}}}] ## sum after type conversion of field
            mydoc=(mycol.aggregate(pipeline=pipe))            
            t="--"
            for x in mydoc:
                t=x['total'].strftime('%Y-%b-%d') 
...
def get_paid_date_of_claim(value1):
...
def get_service_date_of_claim(value1):
    ...
    try:        
        ...
        print(n_records)
        if int(n_records) > 0:
            pipe = [{'$match':{"root_claim_num":str(claim_num_rec)}},{'$group': {'_id':'$root_claim_num', 'total': {'$max': {'$convert':{'input':'$from_date','to':'date'}}}}}] ## sum after type conversion of field
            mydoc=(mycol.aggregate(pipeline=pipe))            
            t="--"
            for x in mydoc:
                t=x['total'].strftime('%Y-%b-%d') 
...
############# high dollar claims chat questions response using search questions call ###############

def chat_get_hgh_dlr_prtl_denied_claims_cnt(dur_list,provider_lst):
    try:
        dt=dur_list
        res=get_hgh_dlr_prtl_dend_claim_vol_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        count=0
        for c_i in val_lst_rcvd:
            try:
        ...
        recvd_num=str(count)+" high dollar Claims were Partially Denied "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

def chat_get_hgh_dlr_denied_claims_cnt(dur_list,provider_lst):
    try:
        dt=dur_list
        res=get_hgh_dlr_dend_claim_vol_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        count=0
        for c_i in val_lst_rcvd:
            try:
        ...
        recvd_num=str(count)+" high dollar Claims were Denied "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

def chat_get_hgh_dlr_adjusted_claims_cnt(dur_list,provider_lst):
    try:
        dt=dur_list
        res=get_hgh_dlr_adjstd_claim_vol_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        count=0
        for c_i in val_lst_rcvd:
            try:
        ...
        recvd_num=str(count)+" high dollar Claims were Adjusted "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

def chat_get_hgh_dlr_received_claims(dur_list,provider_lst):
    try:
        dt=dur_list
        res=get_hgh_dlr_rcvd_claim_vol_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        count=0
        for c_i in val_lst_rcvd:
            try:
        ...
        recvd_num=str(count)+" high dollar Claims were received "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

def chat_get_hgh_dlr_paid_claims(dur_list,provider_lst):
    try:
        dt=dur_list
        res=get_hgh_dlr_paid_claim_vol_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        count=0
        for c_i in val_lst_rcvd:
            try:
        ...
        recvd_num=str(count)+" high dollar Claims were paid "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

def chat_get_hgh_dlr_prtl_denied_submsn_mode_claims(dur_list,provider_lst):
    
    try:
        dt=dur_list
        res=get_hgh_dlr_prtl_dend_claim_submsn_mode_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        hdr_lst_rcvd=list(res["Header"])
        count=""        
        for c_i in range(0,len(val_lst_rcvd)):
            try:
                count=count+val_lst_rcvd[c_i]+" high dollar claims were submitted on "+hdr_lst_rcvd[c_i]+","
            except:
                # count=count+0
                junk=1
        count=count[0:len(count)-1]
...
def chat_get_hgh_dlr_denied_submsn_mode_claims(dur_list,provider_lst):
    
    try:
        dt=dur_list
        res=get_hgh_dlr_dend_claim_submsn_mode_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        hdr_lst_rcvd=list(res["Header"])
        count=""        
        for c_i in range(0,len(val_lst_rcvd)):
...
def chat_get_hgh_dlr_adjusted_submsn_mode_claims(dur_list,provider_lst):
    
    try:
        dt=dur_list
        res=get_hgh_dlr_adjstd_claim_submsn_mode_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        hdr_lst_rcvd=list(res["Header"])
        count=""        
        for c_i in range(0,len(val_lst_rcvd)):
...
def chat_get_hgh_dlr_received_submsn_mode_claims(dur_list,provider_lst):
    
    try:
        dt=dur_list
        res=get_hgh_dlr_rcvd_claim_submsn_mode_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        hdr_lst_rcvd=list(res["Header"])
        count=""        
        for c_i in range(0,len(val_lst_rcvd)):
...
def chat_get_hgh_dlr_paid_submsn_mode_claims(dur_list,provider_lst):
    
    try:
        dt=dur_list
        res=get_hgh_dlr_paid_claim_submsn_mode_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        hdr_lst_rcvd=list(res["Header"])
        count=""        
        for c_i in range(0,len(val_lst_rcvd)):
...
def chat_get_hgh_dlr_prtl_denied_lob_claims(dur_list,provider_lst):
    
    try:
        dt=dur_list
        res=get_hgh_dlr_prtl_dend_claim_lob_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        hdr_lst_rcvd=list(res["Header"])
        count=""        
        for c_i in range(0,len(val_lst_rcvd)):
        ...
        recvd_num=str(count)+" Claims were high dollar partially denied "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

def chat_get_hgh_dlr_denied_lob_claims(dur_list,provider_lst):
    
    try:
        dt=dur_list
        res=get_hgh_dlr_dend_claim_lob_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        hdr_lst_rcvd=list(res["Header"])
        count=""        
        for c_i in range(0,len(val_lst_rcvd)):
        ...
        recvd_num=str(count)+" Claims were high dollar denied "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

def chat_get_hgh_dlr_received_lob_claims(dur_list,provider_lst):
    
    try:
        dt=dur_list
        res=get_hgh_dlr_rcvd_claim_lob_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        hdr_lst_rcvd=list(res["Header"])
        count=""        
        for c_i in range(0,len(val_lst_rcvd)):
        ...
        recvd_num=str(count)+" Claims were high dollar received "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

def chat_get_hgh_dlr_adjusted_lob_claims(dur_list,provider_lst):
    
    try:
        dt=dur_list
        res=get_hgh_dlr_adjstd_claim_lob_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        hdr_lst_rcvd=list(res["Header"])
        count=""        
        for c_i in range(0,len(val_lst_rcvd)):
        ...
        recvd_num=str(count)+" Claims were high dollar adjusted "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

def chat_get_hgh_dlr_paid_lob_claims(dur_list,provider_lst):
    
    try:
        dt=dur_list
        res=get_hgh_dlr_paid_claim_lob_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        hdr_lst_rcvd=list(res["Header"])
        count=""        
        for c_i in range(0,len(val_lst_rcvd)):
        ...
        recvd_num=str(count)+" Claims were high dollar paid "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

def chat_get_hgh_dlr_prtl_denied_par_claims(dur_list,provider_lst):
    
    try:
        dt=dur_list
        res=get_hgh_dlr_prtl_dend_claim_par_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        hdr_lst_rcvd=list(res["Header"])
        count=""        
        for c_i in range(0,len(val_lst_rcvd)):
        ...
        recvd_num=str(count)+" were high dollar partially denied "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

def chat_get_hgh_dlr_denied_par_claims(dur_list,provider_lst):
    
    try:
        dt=dur_list
        res=get_hgh_dlr_dend_claim_par_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        hdr_lst_rcvd=list(res["Header"])
        count=""        
        for c_i in range(0,len(val_lst_rcvd)):
        ...
        recvd_num=str(count)+" were high dollar denied "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

def chat_get_hgh_dlr_adjusted_par_claims(dur_list,provider_lst):
    
    try:
        dt=dur_list
        res=get_hgh_dlr_adjstd_claim_par_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        hdr_lst_rcvd=list(res["Header"])
        count=""        
        for c_i in range(0,len(val_lst_rcvd)):
        ...
        recvd_num=str(count)+" were high dollar adjusted "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

def chat_get_hgh_dlr_received_par_claims(dur_list,provider_lst):
    
    try:
        dt=dur_list
        res=get_hgh_dlr_rcvd_claim_par_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        hdr_lst_rcvd=list(res["Header"])
        count=""        
        for c_i in range(0,len(val_lst_rcvd)):
        ...
        recvd_num=str(count)+" were high dollar received "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

def chat_get_hgh_dlr_paid_par_claims(dur_list,provider_lst):
    
    try:
        dt=dur_list
        res=get_hgh_dlr_paid_claim_par_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        hdr_lst_rcvd=list(res["Header"])
        count=""        
        for c_i in range(0,len(val_lst_rcvd)):
        ...
        recvd_num=str(count)+" were high dollar paid "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

def chat_get_hgh_dlr_prtl_denied_type_claims(dur_list,provider_lst):
    
    try:
        dt=dur_list
        res=get_hgh_dlr_prtl_dend_claim_type_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        hdr_lst_rcvd=list(res["Header"])
        count=""        
        for c_i in range(0,len(val_lst_rcvd)):
        ...
        recvd_num=str(count)+" were high dollar partial denied "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

def chat_get_hgh_dlr_denied_type_claims(dur_list,provider_lst):
    
    try:
        dt=dur_list
        res=get_hgh_dlr_dend_claim_type_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        hdr_lst_rcvd=list(res["Header"])
        count=""        
        for c_i in range(0,len(val_lst_rcvd)):
...
def chat_get_hgh_dlr_received_type_claims(dur_list,provider_lst):
    
    try:
        dt=dur_list
        res=get_hgh_dlr_rcvd_claim_type_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        hdr_lst_rcvd=list(res["Header"])
        count=""        
        for c_i in range(0,len(val_lst_rcvd)):
...
def chat_get_hgh_dlr_adjusted_type_claims(dur_list,provider_lst):
    
    try:
        dt=dur_list
        res=get_hgh_dlr_adjstd_claim_type_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        hdr_lst_rcvd=list(res["Header"])
        count=""        
        for c_i in range(0,len(val_lst_rcvd)):
...
def chat_get_hgh_dlr_paid_type_claims(dur_list,provider_lst):
    
    try:
        dt=dur_list
        res=get_hgh_dlr_paid_claim_type_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        hdr_lst_rcvd=list(res["Header"])
        count=""        
        for c_i in range(0,len(val_lst_rcvd)):
...
def chat_get_hgh_dlr_denial_reasons_trend_bar_archiv(dur_list,provider_lst):
    print("entered denial reasons trend bar for chat")
    try:
    # if True:
        # dt=dur_list
        print(str(dur_list))
        res=get_hgh_dlr_denial_reasons_trend_bar(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        hdr_lst_rcvd=list(res["Header"])
        print(str(val_lst_rcvd))
        print(str(hdr_lst_rcvd))
...
def chat_get_hgh_dlr_denial_reasons_trend_bar(dur_list,provider_lst):
...
def chat_get_hgh_dlr_prtl_denial_reasons_trend_bar(dur_list,provider_lst):
    
    try:
        dt=dur_list
        res=get_hgh_dlr_prtl_denial_reasons_trend_bar(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        hdr_lst_rcvd=list(res["Header"])
        count=""        
        for c_i in range(0,len(val_lst_rcvd)):
...
def chat_get_hgh_dlr_adjstd_reasons_trend_bar(dur_list,provider_lst):
    
    try:
        dt=dur_list
        res=get_hgh_dlr_adjstd_reasons_trend_bar(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        hdr_lst_rcvd=list(res["Header"])
        count=""        
        for c_i in range(0,len(val_lst_rcvd)):
...
def chat_get_hgh_dlr_recvd_billed_amt_claims(dur_list,provider_lst):
    try:
        dt=dur_list        
        res=get_hgh_dlr_rcvd_claim_val_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        count=0
        for c_i in val_lst_rcvd:
            try:
        ...
        recvd_num="The total billed amount for high dollar received claims was "+str(count)+" "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

def chat_get_hgh_dlr_recvd_paid_amt_claims(dur_list,provider_lst):
    try:
        dt=dur_list        
        res=get_hgh_dlr_rcvd_claim_paid_val_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        count=0
        for c_i in val_lst_rcvd:
            try:
        ...
        recvd_num="The total paid amount for high dollar received claims was "+str(count)+" "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

def chat_get_hgh_dlr_paid_paid_amt_claims(dur_list,provider_lst):
    try:
        dt=dur_list        
        res=get_hgh_dlr_paid_claim_val_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        count=0
        for c_i in val_lst_rcvd:
            try:
        ...
        recvd_num="The total paid amount for high dollar paid claims was "+str(count)+" "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

def chat_get_hgh_dlr_paid_billed_amt_claims(dur_list,provider_lst):
    try:
        dt=dur_list        
        res=get_hgh_dlr_paid_claim_val_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        ...
        recvd_num="The total billed amount for high dollar paid claims was "+str(count)+" "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

def chat_get_hgh_dlr_dend_billed_amt_claims(dur_list,provider_lst):
    try:
        dt=dur_list        
        res=get_hgh_dlr_dend_claim_val_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        count=0
        for c_i in val_lst_rcvd:
            try:
        ...
        recvd_num="The total billed amount for high dollar denied claims was "+str(count)+" "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

def chat_get_hgh_dlr_dend_dend_amt_claims(dur_list,provider_lst):
    try:
        dt=dur_list        
        res=get_hgh_dlr_dend_claim_val_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        ...
        recvd_num="The total denied amount for high dollar denied claims was "+str(count)+" "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

def chat_get_hgh_dlr_prtl_dend_billed_amt_claims(dur_list,provider_lst):
    try:
        dt=dur_list        
        res=get_hgh_dlr_prtl_dend_claim_val_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        count=0
        for c_i in val_lst_rcvd:
            try:
        ...
        recvd_num="The total billed amount for high dollar partial denied claims was }"+str(count)+"} "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

def chat_get_hgh_dlr_prtl_dend_paid_amt_claims(dur_list,provider_lst):
    try:
        dt=dur_list        
        res=get_hgh_dlr_prtl_dend_claim_paid_val_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        count=0
        for c_i in val_lst_rcvd:
            try:
        ...
        recvd_num="The total paid amount for high dollar partial denied claims was }"+str(count)+"} "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

def chat_get_hgh_dlr_prtl_dend_dend_amt_claims(dur_list,provider_lst):
    try:
        dt=dur_list        
        count_b=float(chat_get_hgh_dlr_prtl_dend_billed_amt_claims(dur_list,provider_lst).split('}')[1])
        count_p=float(chat_get_hgh_dlr_prtl_dend_paid_amt_claims(dur_list,provider_lst).split('}')[1])
        count=0
        count=count_b-count_p        
        if len(dur_list)==4:
            if len(dt[1]) > 0:
        ...
        recvd_num="The total denied amount for high dollar partial denied claims was "+str(count)+" "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

def chat_get_hgh_dlr_adjstd_paid_amt_claims(dur_list,provider_lst):
    try:
        dt=dur_list        
        res=get_hgh_dlr_adjstd_claim_val_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        count=0
        for c_i in val_lst_rcvd:
            try:
        ...
        recvd_num="The total paid amount for high dollar adjusted claims was }"+str(count)+"} "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

def chat_get_hgh_dlr_adjstd_billed_amt_claims(dur_list,provider_lst):
    try:
        dt=dur_list        
        res=get_hgh_dlr_adjstd_claim_billed_val_trend(dur_list,provider_lst)
        val_lst_rcvd=list(res["Value"])
        count=0
        for c_i in val_lst_rcvd:
            try:
        ...
        recvd_num="The total billed amount for high dollar adjusted claims was }"+str(count)+"} "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

def chat_get_hgh_dlr_adjstd_adjstd_amt_claims(dur_list,provider_lst):
    try:
        dt=dur_list        
        count_b=float(chat_get_hgh_dlr_adjstd_billed_amt_claims(dur_list,provider_lst).split('}')[1])
        count_p=float(chat_get_hgh_dlr_adjstd_paid_amt_claims(dur_list,provider_lst).split('}')[1])
        count=0
        count=count_b-count_p        
        if len(dur_list)==4:
            if len(dt[1]) > 0:
        ...
        recvd_num="The total adjusted amount for high dollar adjusted claims was "+str(count)+" "+dy_op+mon_op+yr_op+q_op+"."
        return recvd_num
    except Exception as e:
        print("Exception in count deriving function="+str(e))

############# high dollar chat questions response using search questions call ###############
