...
def parse():
    ...
    try:
        ...
        try:
            ...
            req_from_gen=0
        try:
            req_type=str(str(parsed.query).split('&')[5].split('=')[1].replace('+',' ')) # second query
        except:
            req_type_gen=0
        if req_from.lower()=='search':
            ans=main_fun(userid_ang,sess_id_ang,s1,provider_lst)
            return jsonify(ans)
        ...
        if s1.lower()=="yes":
            ...
            elif ts == "Queries related to prior auth":
                s1 = "status of PA request "+str((pop_context_hist_claim(userid_ang,sess_id_ang))[5])
        elif s1.lower()=="no":
            ts=pop_conv_hist(userid_ang,sess_id_ang)
            if ts=="Queries related to claims":
                conv_uid=userid_ang
        ...
        if context_claim_r_claims=='CLAIM':
            ...
            if 'prior_auth' not in user_message.lower():
...
if __name__ == "__main__":
    app.run(host='apsrp03693',port=7011,debug=True)