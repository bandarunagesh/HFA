...
def parse():
    ...
    try:
        ...
        if req_from.lower()=='search' and req_type.lower()=='prior_auth':
            ans = pa_main_func(userid_ang,sess_id_ang,s1,provider_lst)
            return jsonify(ans)
        if req_from.lower()=='search':
            ans=main_fun(userid_ang,sess_id_ang,s1,provider_lst,req_chart_type)
            return jsonify(ans)
        ...
        if len(s1.strip(' ')) > 0 and "status" not in s1.lower() and "claim" not in s1.lower():
