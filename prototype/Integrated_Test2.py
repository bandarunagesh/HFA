...
def parse():
    ...
    try:
            ...
            if req_type.lower()=="prior_auth":
                if len(dte_text_lst)==0 and str(claim_num)=='0':
                    ...
                    if len(ret_cntxt) > 0:  
                        ...
                        conv_like_r_dislike="default"
                        conv_obj_id=pa_conv_hist(conv_uid,conv_sesid,conv_question,conv_answer,conv_intent,conv_int_conf,conv_like_r_dislike)
                        return jsonify({"response":"Please enter claim number for hsc Id for PA"})  
            else:
                # changed after integration of pA
                if top_present(user_message):
                    user_message=user_message+" claims"
        ...
        if str(context_claim_r_claims).upper()=="CLAIMS":   
            print("context_claim_r_claims value after time and claim number extraction "+str(context_claim_r_claims))
            print(user_message.lower())
            # if "high" in user_message.lower() and "dollar" in user_message.lower():
            #     context_context="HIGH DOLLAR"         
            if "partial" in user_message.lower() and "denied" in user_message.lower():
                context_context="PARTIALLY DENIED"
                ...
                context_context="ADJUDICATED" 
            elif "received" in user_message.lower() or "submitted" in user_message.lower():
                ...
                if len(ret_cntxt) > 0:  
                    ...
                    if context_context.lower()=="default":
                        context_context="RECEIVED"   
            print("context_context value after time and claim number extraction "+str(context_context))  
        ...
        if context_claim_r_claims=='CLAIM':
            ...
            if " status " in user_message:
                user_message=user_message.replace(" claim "," ").replace(" status "," status of claim ").strip(' ')
        ...
        ##### addding claim/claims accordingly #######
        if req_type.lower() != "prior_auth":
            ######## changed after prior auth integration ############
            if "claims" in user_message or context_claim_r_claims.lower()=="claims":
                user_message=correct_sentence_for_claims(user_message)
            ######## changed after prior auth integration ############
            ...
            if LOB_present(user_message):
                if res_intent.lower()=="claims_lob" and context_claim_r_claims.lower()=="claims":
                    res_intent="claims_lob"
                else:                
                    res_intent="LOB_OF_CLAIM".lower()
            
            if res_intent.lower()=="CLAIM_AMOUNT".lower() and context_claim_r_claims.lower()=="claims":
                response = requests.get("http://apsrp03693:5088/parse",params={"q":user_message.replace(' got ',' is ')}) 
                response = response.json()   
                intent = response.get("intent")
                res_intent=intent['name']
                res_intent_confidence=intent['confidence']            

            if res_intent.lower()=="claim_tax_id".lower():
                user_message=(" "+user_message+" ")
                if "tax id" not in user_message:
                    user_message=user_message.replace(" id "," ").strip(' ')
                response = requests.get("http://apsrp03693:5088/parse",params={"q":user_message}) 
                response = response.json()   
                intent = response.get("intent")
                res_intent=intent['name']
            ...
            if res_intent.upper().strip(' ')=="STATUS":
                ...
                if context_context=="PARTIALLY DENIED":
                    ans=chat_get_prtl_denied_claims_cnt(dur_list,provider_lst)
                elif context_context=="DENIED" :
                    ans=chat_get_denied_claims_cnt(dur_list,provider_lst)
                elif context_context=="ADJUSTED":
                    ans=chat_get_adjusted_claims_cnt(dur_list,provider_lst)
                elif context_context=="RECEIVED" :
                    ans=chat_get_received_claims(dur_list,provider_lst)
                elif context_context=="PAID":
                    ans=chat_get_paid_claims(dur_list,provider_lst)
                ans=ans.replace('  in month of January,  February,  March,  April,  May,  June,  July,  August,  September,  October,  November,  December','')
            elif res_intent.upper().strip(' ')== "claims_billed_amt".upper():
                dur_list=list()
                if context_from.upper()=="default".upper():
                    dur_list.append(context_days)
                    dur_list.append(context_months)
                    ...
                    dur_list.append("junk")
                if context_context=="PARTIALLY DENIED":
                    ans=chat_get_prtl_dend_billed_amt_claims(dur_list,provider_lst)
                elif context_context=="DENIED" :
                    ans=chat_get_dend_billed_amt_claims(dur_list,provider_lst)
                elif context_context=="ADJUSTED":
                    ans=chat_get_adjstd_billed_amt_claims(dur_list,provider_lst)
                elif context_context=="RECEIVED" :
                    ans=chat_get_recvd_billed_amt_claims(dur_list,provider_lst)
                elif context_context=="PAID":
                    ans=chat_get_paid_billed_amt_claims(dur_list,provider_lst)
                ans=ans.replace('  in month of January,  February,  March,  April,  May,  June,  July,  August,  September,  October,  November,  December','').replace('}','')
            elif res_intent.upper().strip(' ')== "claims_paid_amt".upper():
                dur_list=list()
                ...
                if "partial" in user_message.lower() and "denied" in user_message.lower():
                    context_context="PARTIALLY DENIED"
                    ans=chat_get_prtl_dend_paid_amt_claims(dur_list,provider_lst)
                elif "denied" in user_message.lower():
                    context_context=="DENIED" 
                    ans="Denied Claims will not be paid"
                elif "adjusted" in user_message.lower():
                    context_context="ADJUSTED"
                    ans=chat_get_adjstd_paid_amt_claims(dur_list,provider_lst)
                elif "received" in user_message.lower():
                    context_context="RECEIVED" 
                    ans=chat_get_recvd_paid_amt_claims(dur_list,provider_lst)
                else:
                    if (user_message.lower().split(' ').count('paid')) > 1:
                        context_context="PAID"
                        ans=chat_get_paid_paid_amt_claims(dur_list,provider_lst)
                    else:
                        ret_cntxt_1=pop_context_hist_claims(context_uid,context_sessid)
                        if len(ret_cntxt_1)>0:
                            context_context=ret_cntxt_1[4]
                        else:
                            context_context="RECEIVED" 
            ...
            elif res_intent.upper().strip(' ')== "claims_dend_amt".upper():
                dur_list=list()
                ...
                if "partial" in user_message.lower() and "denied" in user_message.lower():
                    context_context="PARTIALLY DENIED"
                    ans=chat_get_prtl_dend_dend_amt_claims(dur_list,provider_lst)
                elif "paid" in user_message.lower():
                    context_context=="PAID" 
                    ans="There will be no denials for Paid Claims"
                elif "adjusted" in user_message.lower():
                    context_context="ADJUSTED"
                    ans="There will be no adjustments for Paid Claims"
                elif "received" in user_message.lower():
                    context_context="RECEIVED" 
                    ans=chat_get_dend_dend_amt_claims(dur_list,provider_lst)
                else:
                    if (user_message.lower().split(' ').count('denied')) > 1:
                        context_context="DENIED"
                        ans=chat_get_dend_dend_amt_claims(dur_list,provider_lst)
                    else:
                        ret_cntxt_1=pop_context_hist_claims(context_uid,context_sessid)
            ...
            elif res_intent.upper().strip(' ')== "claims_adjstd_amt".upper():
                dur_list=list()
                ...
                if "partial" in user_message.lower() and "denied" in user_message.lower():
                    context_context="PARTIALLY DENIED"
                    ans="There will be no adjustments for Partial Denied Claims"
                elif "paid" in user_message.lower():
                    context_context=="PAID" 
                    ans=chat_get_adjstd_paid_amt_claims(dur_list,provider_lst)
                elif "denied" in user_message.lower():
                    context_context="DENIED"
                    ans="There will be no adjustments for Denied Claims"
                elif "received" in user_message.lower():
                    context_context="RECEIVED" 
                    ans=chat_get_adjstd_adjstd_amt_claims(dur_list,provider_lst)
                else:
                    if (user_message.lower().split(' ').count('adjusted')) > 1:
                        context_context="ADJUSTED"
                        ans=chat_get_adjstd_adjstd_amt_claims(dur_list,provider_lst)
                    else:
                        ret_cntxt_1=pop_context_hist_claims(context_uid,context_sessid)
            ...
            elif res_intent.upper().strip(' ')=="CLAIMS_SUBMSN_MODE":
                dur_list=list()
                if context_from.upper()=="default".upper():
                    dur_list.append(context_days)
                    dur_list.append(context_months)
                ...
                if context_context=="PARTIALLY DENIED":
                    ans=chat_get_prtl_denied_submsn_mode_claims(dur_list,provider_lst)
                elif context_context=="DENIED" :
                    ans=chat_get_denied_submsn_mode_claims(dur_list,provider_lst)
                elif context_context=="ADJUSTED":
                    ans=chat_get_adjusted_submsn_mode_claims(dur_list,provider_lst)
                elif context_context=="RECEIVED" :
                    ans=chat_get_received_submsn_mode_claims(dur_list,provider_lst)
                elif context_context=="PAID":
                    ans=chat_get_paid_submsn_mode_claims(dur_list,provider_lst)
                ans=ans.replace('  in month of January,  February,  March,  April,  May,  June,  July,  August,  September,  October,  November,  December','')
            elif res_intent.upper().strip(' ')=="CLAIMS_LOB":
                print("contexts while disatching"+str(context_context))
                dur_list=list()
                if context_from.upper()=="default".upper():
                    dur_list.append(context_days)
                    dur_list.append(context_months)
                ...
                if context_context=="PARTIALLY DENIED":
                    ans=chat_get_prtl_denied_lob_claims(dur_list,provider_lst)
                elif context_context=="DENIED" :
                    ans=chat_get_denied_lob_claims(dur_list,provider_lst)
                elif context_context=="ADJUSTED":
                    ans=chat_get_adjusted_lob_claims(dur_list,provider_lst)
                elif context_context=="RECEIVED" :
                    ans=chat_get_received_lob_claims(dur_list,provider_lst)
                elif context_context=="PAID":
                    ans=chat_get_paid_lob_claims(dur_list,provider_lst)
                print(str(ans))
                ans=ans.replace('  in month of January,  February,  March,  April,  May,  June,  July,  August,  September,  October,  November,  December','') 
            elif res_intent.upper().strip(' ')=="CLAIMS_PAR_NON_PAR":
                dur_list=list()
                if context_from.upper()=="default".upper():
                    dur_list.append(context_days)
                    dur_list.append(context_months)
                ...
                if context_context=="PARTIALLY DENIED":
                    ans=chat_get_prtl_denied_par_claims(dur_list,provider_lst)
                elif context_context=="DENIED" :
                    ans=chat_get_denied_par_claims(dur_list,provider_lst)
                elif context_context=="ADJUSTED":
                    ans=chat_get_adjusted_par_claims(dur_list,provider_lst)
                elif context_context=="RECEIVED" :
                    ans=chat_get_received_par_claims(dur_list,provider_lst)
                elif context_context=="PAID":
                    ans=chat_get_paid_par_claims(dur_list,provider_lst)
                ans=ans.replace('  in month of January,  February,  March,  April,  May,  June,  July,  August,  September,  October,  November,  December','') 
            elif res_intent.upper().strip(' ')=="CLAIMS_TYPE":
                dur_list=list()
                if context_from.upper()=="default".upper():
                    dur_list.append(context_days)
                    dur_list.append(context_months)
                ...
                if context_context=="PARTIALLY DENIED":
                    ans=chat_get_prtl_denied_type_claims(dur_list,provider_lst)
                elif context_context=="DENIED" :
                    ans=chat_get_denied_type_claims(dur_list,provider_lst)
                elif context_context=="ADJUSTED":
                    ans=chat_get_adjusted_type_claims(dur_list,provider_lst)
                elif context_context=="RECEIVED" :
                    ans=chat_get_received_type_claims(dur_list,provider_lst)
                elif context_context=="PAID":
                    ans=chat_get_paid_type_claims(dur_list,provider_lst)
                ans=ans.replace('  in month of January,  February,  March,  April,  May,  June,  July,  August,  September,  October,  November,  December','')
            elif res_intent.upper().strip(' ')=="claims_action_reasons".upper():
                print("entered claims_actions_reasons if block")
                print(str(context_context))
                dur_list=list()
                if context_from.upper()=="default".upper():
                    dur_list.append(context_days)
                    dur_list.append(context_months)
                ...
                if "partial" in user_message.lower() and "denied" in user_message.lower():
                    context_context=="PARTIALLY DENIED"
                    print("entered partial denied")
                    ans=chat_get_prtl_denial_reasons_trend_bar(dur_list,provider_lst)
                # elif context_context=="DENIED" :
                elif  "denied" in user_message.lower():
                    print("entered denied")
                    context_context=="DENIED"
                    ans=chat_get_denial_reasons_trend_bar(dur_list,provider_lst)
                # elif context_context=="ADJUSTED":
                elif  "adjusted" in user_message.lower():
                    print("adjusted")
                    context_context=="ADJUSTED"
                    ans=chat_get_adjstd_reasons_trend_bar(dur_list,provider_lst)
                ans=ans.replace('  in month of January,  February,  March,  April,  May,  June,  July,  August,  September,  October,  November,  December','')     
            else:
            ...
            for si in range(0,len(sugg_list)):
                if sugg_list[si].lower()=="Date of service".lower():
                    ...
                    sugg_list[si]="What are the reasons for denials in the claim?"
                elif sugg_list[si].lower()=="Allowed Amount".lower():
                    sugg_list[si]="How much amount is Allowed for claim?"
