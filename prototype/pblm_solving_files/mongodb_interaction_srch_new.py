...
def main_fun(userid,sessid,req,prov_lst,req_chart_type):
    ...
    if len(dur_lst)==0:
        ...
        try:
            ...
            yr_lst=[]
            yr_lst.append(str(datetime.datetime.now().year))
            m_lst=list()
            for mi in range(0,int(str((datetime.datetime.now().month)))):
                m1=mi+1
                if m1 < 10:
    ...
    try:
        ...
        if len(dur_lst)==3:
            ...
            while True:
                ...
                cond=cond+"{'received_date_str':{'$regex':'.*-"+str(mon_frmt)+"-.*'}}"
                cond=cond+",{'received_date_str':{'$regex':'"+str(yr_strt)+"-.*-.*'}}"
            ...
            for dl in dur_lst[2]:
                for dl1 in dur_lst[1]:
                    cond=cond+"{'received_date_str':{'$regex':'.*-"+str(dl1)+"-.*'}}"
                    cond=cond+",{'received_date_str':{'$regex':'"+str(dl)+"-.*-.*'}}"
                    if len(provider_lst) > 0:
                        pc=",{'payto_provider_tax_id':{'$in':"+str(provider_lst)+"}}"
                        cond="{'$and':["+cond+pc+"]}"
                    else:
        ...
        if len(dur_lst)==3:
            ...
            while True:
                ...
                if mon_strt < 10:
                    ...
                    mon_frmt=str(mon_strt)
                cond=cond+"{'paid_date_str':{'$regex':'.*-"+str(mon_frmt)+"-.*'}}"
                cond=cond+",{'paid_date_str':{'$regex':'"+str(yr_strt)+"-.*-.*'}}"
                if len(provider_lst) > 0:
                    pc=",{'payto_provider_tax_id':{'$in':"+str(provider_lst)+"}}"
            ...
            for dl in dur_lst[2]:
                for dl1 in dur_lst[1]:
                    cond=cond+"{'paid_date_str':{'$regex':'.*-"+str(dl1)+"-.*'}}"
                    cond=cond+",{'paid_date_str':{'$regex':'"+str(dl)+"-.*-.*'}}"
                    cond="{'$and':["+cond+"]}"
                    if len(provider_lst) > 0:
                        pc=",{'payto_provider_tax_id':{'$in':"+str(provider_lst)+"}}"
                        di=",{'claim_denial_ind':'0'}"
