...
def push_context_hist(userid,sess_id,req_type,claims_r_claim,context,claim_num,frm,to,day,mon,yr):
    ...
    mycol = mydb["Context_History"]
    dt=str(datetime.now()).replace(' ','').replace('-','').replace(':','')
    mydict = { "User_ID": userid, "Session_ID": sess_id,"Request_Type":req_type,"Claim_R_Claims":claims_r_claim,"Context":context,"Claim_num":claim_num,"From":frm,"To":to,"Days":day,"Months":mon,"Years":yr,"creation_dt":dt}
    x = mycol.insert_one(mydict)
    return str(x.inserted_id)

def pop_context_hist(userid,sess_id):
    ...
    for x in mydoc:
        ...
        response_lst.append(x['Claim_num'])
...
def pop_context_hist_claim(userid,sess_id):
    print("in context history pop function")
    myclient = pymongo.MongoClient(u"mongodb://apsrp03693.uhc.com:27017")
    print(myclient.list_database_names())
    mydb = myclient["HandsFreeAnalytics"]
    print(mydb.list_collection_names())
    mycol = mydb["Context_History"]
    cond="{\"$and\":[{\"User_ID\":\'"+str(userid)+"\'},{\"Session_ID\":\'"+str(sess_id)+"\'},{\"Claim_R_Claims\":\'"+str("CLAIM")+"\'}]}"
    print(cond)
    my_dict = ast.literal_eval(cond)
    print(my_dict)
    mydoc=mycol.find(my_dict).sort('creation_dt',pymongo.DESCENDING)
...
def pop_context_hist_claims(userid,sess_id):
    print("in context history pop function")
    myclient = pymongo.MongoClient(u"mongodb://apsrp03693.uhc.com:27017")
    print(myclient.list_database_names())
    mydb = myclient["HandsFreeAnalytics"]
    print(mydb.list_collection_names())
    mycol = mydb["Context_History"]
    cond="{\"$and\":[{\"User_ID\":\'"+str(userid)+"\'},{\"Session_ID\":\'"+str(sess_id)+"\'},{\"Claim_R_Claims\":\'"+str("CLAIMS")+"\'}]}"
    print(cond)
    my_dict = ast.literal_eval(cond)
    print(my_dict)
    mydoc=mycol.find(my_dict).sort('creation_dt',pymongo.DESCENDING)
